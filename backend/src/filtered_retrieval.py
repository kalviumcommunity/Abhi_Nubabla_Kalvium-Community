...
class FilteredRetriever:
    ...
    def _load_corpus(self) -> None:
        ...
        candidates = [
            ...
            Path("data/results/indexed_collection.json"),
            Path("data/results/embedded_chunks.json"),
            Path("data/results/ingested_chunks.json"),
        ]
        loaded = False
...
def run_filtered_retrieval_benchmark(
    ...
    if save_artifacts:
        # Export JSON Results
        json_path = Path("data/results/filtered_search_results.json")
        json_path.parent.mkdir(parents=True, exist_ok=True)
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(suite_results, f, indent=2, ensure_ascii=False)

        # Export Markdown Report
        md_path = Path("data/results/filtered_search_report.md")
