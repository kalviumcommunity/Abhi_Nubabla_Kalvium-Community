...
class RetrievalEvaluator:
    ...
    def run_full_comparative_benchmark(
        ...
        if save_artifacts:
            ...
            json_path = Path("data/results/retrieval_evaluation_results.json")
            json_path.parent.mkdir(parents=True, exist_ok=True)
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(full_benchmark_output, f, indent=2, ensure_ascii=False)

            # 2. Export Markdown Report
            md_path = Path("data/results/retrieval_evaluation_report.md")
    ...
    if args.mode == "all":
        ...
        if console:
            ...
            if not args.no_save:
                ...
                console.print(f"  • JSON Dataset: [cyan]data/results/retrieval_evaluation_results.json[/cyan]")
                console.print(f"  • Markdown Audit: [cyan]data/results/retrieval_evaluation_report.md[/cyan]\n")
        else:
