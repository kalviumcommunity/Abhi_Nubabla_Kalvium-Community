...
def run_grounded_generation_benchmark(
    ...
    if save_artifacts:
        ...
        json_path = Path("data/results/grounded_generation_results.json")
        json_path.parent.mkdir(parents=True, exist_ok=True)
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(benchmark_data, f, indent=2, ensure_ascii=False)

        # 2. Export Markdown Report
        md_path = Path("data/results/grounded_generation_report.md")
...
def main():
    ...
    if args.query:
        ...
        if console:
            ...
            console.print("  • JSON Dataset: [cyan]data/results/grounded_generation_results.json[/cyan]")
            console.print("  • Markdown Audit: [cyan]data/results/grounded_generation_report.md[/cyan]")
        else:
