...
# ---------------------------------------------------------------------------
def load_corpus_chunks(chunks_path: str = "data/results/ingested_chunks.json") -> List[Dict[str, Any]]:
    path = Path(chunks_path)
    if not path.exists():
        alt_path = Path("data/results/sample_chunks.json")
        if alt_path.exists():
            path = alt_path
        else:
...
def main():
    ...
    parser.add_argument("--no-save", action="store_true", help="Do not save output JSON/Markdown reports to data/results/")
